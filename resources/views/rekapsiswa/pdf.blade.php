<!DOCTYPE html>
<html>
<head>
	<title>Laporan Data Presensi Mahasiswa</title>
	<style type="text/css">
		table {
			border-style: double;
			border-width: 3px;
			border-color: white;
		}
		table tr .text2 {
			text-align: right;
			font-size: 13px;
		}
		table tr .text {
			text-align: center;
			font-size: 13px;
		}
		table tr td {
			font-size: 13px;
		}
        .footer {
            text-align: right;
            margin-top: 50px;
            display: flex;
            justify-content: flex-start;
        }
        .footer span {
            display: block;
        }
         /* CSS untuk tabel */
         table {
            width: 100%;
            border-collapse: collapse;
        }
        th, td {
            padding: 8px;
            text-align: left;
            border-bottom: 1px solid #ddd;
        }
        th {
            background-color: #f2f2f2;
        }
        body{font-family: Arial;background-color:#fff;}
        .kopsurat{width: 650px;margin: 0 auto;background-color: #fff;height: 500px;padding: 20px:}
        .tengah {text-align: center;libe-height: 7px;}

	</style>
</head>
<body>
	<center>
		<table>
			<tr>
				<td><img src="data:image/jpeg;base64," width="90" height="90"></td>
				<td>
				<center>
					<font size="4">GRAHA PENA SURABAYA</font><br>
					<font size="5"><b>PT OTAK KANAN</b></font><br>
					<font size="2">Bidang Keahlian : Bisnis dan Menejemen - Teknologi informasi dan Komunikasi</font><br>
					<font size="2"><i>Graha Pena suite 1503 Jl. Ahmad Yani 88 Surabaya Jawa Timur – Indonesia</i></font>
				</center>
				</td>
			</tr>
			<tr>
				<td colspan="2">
                    <hr>
                    <center>
                      <font size="4" face="Arial" style="font-weight: bold;">Laporan Rekap Presensi Mahasiswa {{ $siswa->nama }}</font>
                    </center>
                  </td>
			</tr>

		</table>

        @php
    $holidays = [
        ['date' => '2023-12-25', 'name' => 'Hari Raya Natal'],
        ['date' => '2023-09-28', 'name' => 'Maulid Nabi Muhammad SAW'],
        ['date' => '2023-08-17', 'name' => 'Hari Proklamasi Kemerdekaan RI'],
        ['date' => '2023-07-19', 'name' => 'Tahun Baru Islam 1445 Hijriyah'],
        ['date' => '2023-06-29', 'name' => 'Hari Raya Idul Adha 1444 Hijriyah'],
        ['date' => '2023-06-4', 'name' => 'Hari Raya Waisak 2567'],
        ['date' => '2023-06-1', 'name' => 'Hari Lahirnya Pancasila'],
        ['date' => '2023-05-18', 'name' => 'Kenaikan Isa Al Masih'],
        ['date' => '2023-05-1', 'name' => 'Hari Buruh Internasional'],
        ['date' => '2023-04-23', 'name' => 'Hari Raya Idul Fitri 1444 Hijriyah'],
        ['date' => '2023-04-22', 'name' => 'Hari Raya Idul Fitri 1444 Hijriyah'],
        ['date' => '2023-04-7', 'name' => 'Wafat Isa Al Masih'],
        ['date' => '2023-03-22', 'name' => 'Hari Raya Nyepi'],
        ['date' => '2023-02-18', 'name' => 'Isra Mikraj Nabi Muhammad SAW'],
        ['date' => '2023-01-22', 'name' => 'Tahun Baru Imlek 2574 Kongzili'],
        ['date' => '2023-01-1', 'name' => 'Tahun Baru Masehi'],
    ];
@endphp
@php
    $totalPresentase = 0;
    $countData = 0;
@endphp


<center>
    <table>
        <thead>
            <tr>
                <th>ID</th>
                <th>Nama Siswa</th>
                <th>Tanggal</th>
                <th>Hari</th>
                <th>Jam Masuk</th>
                <th>Jam Pulang</th>
                <th>Keterlambatan (menit)</th>
                <th>Status</th>
                <th>Keterangan</th>
                <th>Presentase</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($dateRange as $date)
                @php
                    $absensi = $absensis->where('tanggal', $date)->first();
                    $isHoliday = false;
                    $holidayName = '';
                    $dayOfWeek = \Carbon\Carbon::parse($date)->locale('id')->isoFormat('dddd');

                    foreach ($holidays as $holiday) {
                        if ($holiday['date'] == $date) {
                            $isHoliday = true;
                            $holidayName = $holiday['name'];
                            break;
                        }
                    }
                @endphp

                <tr>
                    <td>{{ $siswa->id }}</td>
                    <td>{{ $siswa->nama }}</td>
                    <td>{{ $date }}</td>
                    <td>{{ \Carbon\Carbon::parse($date)->locale('id')->isoFormat('dddd') }}</td>
                    <td>{{ $absensi ? date('H:i', strtotime($absensi->waktu)) : '-' }}</td>
                    <td>
                        @php
                            if ($absensi) {
                                $jamPulang = date('H:i', strtotime($absensi->waktu . ' +8 hours'));
                                if ($jamPulang < '06:00') {
                                    $jamPulang = date('H:i', strtotime($jamPulang . ' +24 hours'));
                                }
                                echo $jamPulang;
                            } else {
                                echo '-';
                            }
                        @endphp
                    </td>
                    <td>
                        @php
                            if ($absensi && $absensi->status === 'Terlambat') {
                                $jamMasuk = strtotime('08:00');
                                $jamAbsensi = strtotime($absensi->waktu);
                                $keterlambatan = max(0, ($jamAbsensi - $jamMasuk) / 60);
                            } else {
                                $keterlambatan = 0;
                            }
                            echo $keterlambatan;
                        @endphp
                    </td>
                    <td>{{ $absensi ? $absensi->status : '-' }}</td>
                    <td>
                        @if ($isHoliday)
                            {{ $holidayName }}
                        @else
                            -
                        @endif
                    </td>
                    <td>
                        @php
                            if ($absensi) {
                                if ($absensi->status === 'Masuk' || $absensi->status === 'Pulang Tepat Waktu') {
                                    $presentase = 100;
                                } else if ($absensi->status === 'Terlambat') {
                                    $jamMasuk = strtotime('08:00');
                                    $jamAbsensi = strtotime($absensi->waktu);
                                    $keterlambatan = max(0, ($jamAbsensi - $jamMasuk) / 60);
                                    $presentase = 100 - $keterlambatan;
                                } else {
                                    $presentase = 0;
                                }
                                $totalPresentase += $presentase;
                                $countData++;
                                echo number_format($presentase, 2) . '%';
                            } else {
                                $totalPresentase -= 100;
                            }
                        @endphp
                    </td>
                </tr>
            @endforeach
            <tr>
                <td colspan="9">Rata-rata Presentase Kehadiran Siswa</td>
                <td>
                    @php
                        $rataRataPresentase = $countData > 0 ? $totalPresentase / $countData : 0;
                        $rataRataPresentase = abs($rataRataPresentase);
                        echo number_format($rataRataPresentase, 2) . '%';
                    @endphp
                </td>
            </tr>
        </tbody>
    </table>
</center>



        <div class="footer">
            <span>Surabaya, {{ date('d F Y') }}</span>
            <span style="margin-top: 20px;"></span>
            <span style="margin-top: 20px;"></span>
            <span style="margin-top: 20px;"></span>
            <span>(................................)</span>
        </div>
		<br>

	</center>

</body>
</html>
